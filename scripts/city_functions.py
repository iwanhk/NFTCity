...
def whitelist_mint(city, cityDict, nft, user, price):
    ...
    print(f"Minting NFT {city} ...")
    tx= nft.whitelistMint(user, proof[user.address], names_list, zoneDiff, lan_list, addr2(user, price))
    tx.wait(1)
    if tx.status==1:
...
def whitelist_mint_test(city, cityDict, nft, user1, price, user2):
    ...
    print(f"Minting NFT {city} ...")
    tx= nft.whitelistMint(user1, proof[user2.address], names_list, zoneDiff, lan_list, addr2(user1, price))
    tx.wait(1)
    if tx.status==1:
        _after_mint_record(city, nft)

def public_mint(city, cityDict, nft, user, price):
    names_list, zoneDiff, lan_list= _get_meta(city, cityDict)
    if names_list==None:
        return

    print(f"Minting NFT {city} ...")
    tx= nft.publicSaleMint(user, names_list, zoneDiff, lan_list, addr2(user, price))
    tx.wait(1)
    if tx.status==1:
        _after_mint_record(city, nft)
...
def gift(city, cityDict, nft, _to, _from):
    ...
    print(f"Minting NFT {city} ...")
    tx= nft.gift(_to, names_list, zoneDiff, lan_list, addr(_from))
    tx.wait(1)
    if tx.status==1:
        _after_mint_record(city, nft)
