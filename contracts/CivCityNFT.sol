...
contract CivCityNFT is Ownable, ERC721A, PaymentSplitter {
  ...
  function showAnimation(uint256 tokenId, bool _show) public tokenExist(tokenId) ownerOfToken(tokenId) {
      require(bytes(ipfsPrefix).length>0, "Admin has not set the ipfs prefix");
      animation[tokenId] = _show;
  }

  function setFont(uint256 tokenId, string calldata _font) public tokenExist(tokenId) ownerOfToken(tokenId) {
      font[tokenId] = _font;
  }
  
  function setMainLang(uint256 tokenId, string calldata lang) public tokenExist(tokenId) ownerOfToken(tokenId) {
  ...
  function getMainLang(uint256 tokenId) view public tokenExist(tokenId) returns(string memory){
